...
<?php include 'templatecopier.php';?>
...
$custom_domain_enabled = 0; //is_numeric($_POST['custom_domain_status']) ? mysqli_real_escape_string($mysqli, (int) $_POST['custom_domain_status']) : 0;
...
if ($r) {
    ...
    //insert new record
    $q = 'INSERT INTO login (name, company, username, password, tied_to, app, timezone, language, reset_password_key) VALUES ("' . $from_name . '", "' . $app_name . '", "' . $login_email . '", "' . $pass_encrypted . '", ' . 1 . ', ' . $id . ', "' . 'America/New_York' . '", "' . $language . '", "' . $rpk . '")';

    $r = mysqli_query($mysqli, $q);

    
    // get the id of the last commit
    $q = 'SELECT id, company, app FROM login WHERE username = "' . $login_email . '" LIMIT 1';
    $r = mysqli_query($mysqli, $q);
...
//    echo "New token is : ".$token;
    //$q2 = 'UPDATE login SET reset_password_key = "'.$rpk.'" WHERE id = '.$uid;
   // $r2 = mysqli_query($mysqli, $q2);
     
    $token = short('{"rpk":"' . $rpk . '", "id":"' . $uid . '"}');
    validateUser($mysqli, $from_email, $app_name, $token);
	copyTemplates($id,74,$mysqli);
	header("location:login.php");
}
}
header("location:https://ading.us/confirmation"); /* Redirect browser */
exit();
?>
